...
Dates.events = function(){
    ...
    Dates.on(as.oclear, 'click', function(){
        Dates.elem[as.elemv] = '';
        Dates.close();
        /** added by chenyafeng 2017-07-13 **/
        typeof Dates.options.lyclear === 'function' && Dates.options.lyclear(); 
    });
    ...
    Dates.on(as.otoday, 'click', function(){
        ...
        /**选中今天显示不正确 update by chenyafeng 2017-07-13**/
        //Dates.creation([Dates.ymd[0], Dates.ymd[1]+1, Dates.ymd[2]]);
        Dates.creation([now.getFullYear(), now.getMonth() + 1, now.getDate()]);
        /** added by chenyafeng 2017-07-13 **/
        var S = Dates.query, hms = Dates.hmsin;
        var getDates = Dates.parse([Dates.ymd[0], Dates.ymd[1]+1, Dates.ymd[2]], [hms[0].value, hms[1].value, hms[2].value]);
        typeof Dates.options.lytoday === 'function' && Dates.options.lytoday(getDates);
    });
    ...
    Dates.on(as.ok, 'click', function(){
        if(Dates.valid){
            ...
            var S = Dates.query, hms = Dates.hmsin;
            var getDates = Dates.parse([Dates.ymd[0], Dates.ymd[1]+1, Dates.ymd[2]], [hms[0].value, hms[1].value, hms[2].value]);
            typeof Dates.options.lyok === 'function' && Dates.options.lyok(getDates); 
        }
    });
